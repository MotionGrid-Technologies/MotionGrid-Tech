...
export const MARKETING_TEMPLATES: Record<string, TemplateDef> = {
  ...
  meeting_booking_notification: {
    subject: 'New meeting booked: {{name}} ({{company}})',
    text: [
      'A 15-minute meeting was booked on the MotionGrid website.',
      '',
      'Name: {{name}}',
      'Company: {{company}}',
      'Email: {{email}}',
    ...
    html: [
      '<div style="font-family:system-ui,sans-serif;max-width:600px;margin:0 auto;padding:24px;color:#1f2937;">',
      '<h2>{{businessName}}</h2>',
      '<p>A 15-minute meeting was booked on the website:</p>',
      '<table style="width:100%;border-collapse:collapse;margin:16px 0;">',
      '<tr><td style="padding:6px 0;"><strong>Name</strong></td><td>{{name}}</td></tr>',
      '<tr><td style="padding:6px 0;"><strong>Company</strong></td><td>{{company}}</td></tr>',
      '<tr><td style="padding:6px 0;"><strong>Email</strong></td><td>{{email}}</td></tr>',
      '<tr><td style="padding:6px 0;"><strong>Phone</strong></td><td>{{phone}}</td></tr>',
      '<tr><td style="padding:6px 0;"><strong>Slot</strong></td><td>{{slot}}</td></tr>',
      '</table>',
      '<div style="background:#f9fafb;border-radius:8px;padding:16px;margin:16px 0;">',
  ...
  meeting_booking_confirmation: {
    subject: 'Your 15-minute slot is confirmed, {{name}}',
    text: [
      'Hi {{name}},',
      '',
      'Your 15-minute call with {{businessName}} is booked:',
      '',
      '{{slot}}',
      '',
      'One of our founders will join the call — if anything changes, just reply to this email and we will reschedule.',
      '',
      'Talk soon,',
      'The {{businessName}} team',
    ].join('\n'),
    html: [
      ...
      '<h2>{{businessName}}</h2>',
      '<p>Hi {{name}},</p>',
      '<p>Your 15-minute call with us is booked:</p>',
      '<p style="background:#f9fafb;border-radius:8px;padding:16px;font-weight:600;">{{slot}}</p>',
      '<p>One of our founders will join the call — if anything changes, just reply to this email and we will reschedule.</p>',
      '<p style="font-size:12px;color:#9ca3af;margin-top:24px;">&mdash; The {{businessName}} team</p>',
      '</div>',
    ].join(''),
  },
