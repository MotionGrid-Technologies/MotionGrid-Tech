...
export default function SuperAdminWorkshopsPage() {
  ...
  const createFormOpen = showForm || searchParams.get('new') === 'true'

  function closeCreateForm() {
    setShowForm(false)
    if (searchParams.get('new') === 'true') router.replace('/dashboard/admin/autofield/workshops')
  }
  ...
  async function handleStatusChange(workshopId: string, status: string, reason?: string) {
    try {
      ...
      if (isPostHogConfigured) posthog.capture('workshop_status_updated', { status })
      fetchWorkshops()
    } catch (err: unknown) {
  ...
  async function handleDeactivate(workshopId: string) {
    ...
    try {
      ...
      if (isPostHogConfigured) posthog.capture('workshop_deactivated')
      fetchWorkshops()
    } catch (err: unknown) {
  ...
  async function handleEdit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/workshops?id=${editingWorkshop.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (isPostHogConfigured) posthog.capture('workshop_details_updated')
  ...
  return (
    <PageWrapper>
      <div className="space-y-6">
        ...
        {editingWorkshop && (
          <div className="bg-white border border-grey-light rounded-base p-6 shadow-sm">
            <h2 className="text-lg font-bold text-grey mb-4">Edit Workshop</h2>
            <form onSubmit={handleEdit} className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="block text-sm font-semibold text-grey mb-1">Workshop Name</label>
                <input name="name" required defaultValue={editingWorkshop.name} className="w-full rounded-base border border-grey-light px-3 py-2 text-grey focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" />
              </div>
              <div>
                <label className="block text-sm font-semibold text-grey mb-1">URL Slug</label>
                <input name="slug" required defaultValue={editingWorkshop.slug} className="w-full rounded-base border border-grey-light px-3 py-2 text-grey focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" />
              </div>
              <div>
                <label className="block text-sm font-semibold text-grey mb-1">Domain</label>
                <input name="domain" defaultValue={editingWorkshop.domain ?? ''} placeholder="mechanic.co.za" className="w-full rounded-base border border-grey-light px-3 py-2 text-grey focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" />
              </div>
              <div className="md:col-span-3 flex gap-3 justify-end">
                <button type="button" onClick={() => setEditingWorkshop(null)} className="px-4 py-2 text-sm font-semibold text-grey border border-grey-light rounded-base hover:bg-grey-light/20 transition-colors">Cancel</button>
                <button type="submit" disabled={savingEdit} className="flex items-center gap-2 bg-primary text-white px-4 py-2 rounded-base font-semibold text-sm hover:bg-primary-dark transition-colors disabled:opacity-50">
                    ...
                    <tr key={w.id} className="hover:bg-grey-lightest/50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex items-center gap-1.5 justify-end">
                          ...
                          <Link
                            href={`/dashboard/admin/autofield/settings?workshopId=${w.id}`}
                            className="flex items-center gap-1 text-xs text-primary font-semibold hover:underline no-underline"
                          >
