...
- [x] **Rotate all exposed secrets** — Resend `re_...` (shared in chat) + Supabase anon/service keys (present in git history)
...
- [x] Create Supabase project + connect to Vercel
- [x] Run initial schema migration in Supabase SQL Editor
- [x] Set `SUPABASE_URL` + `SUPABASE_ANON_KEY` env vars in Vercel
- [x] Set `SUPABASE_SERVICE_ROLE_KEY` (server-only, never exposed to client)
...
### 1.3 Social Proof
- [x] Testimonials page (`/testimonials`) — editorial quotes + DB-backed approved reviews
- [x] Client logos marquee (typeset wordmarks; swap in real SVGs when available)
- [x] Case studies page (`/case-studies` + `/case-studies/[slug]`)
- [x] Review system with admin approval (`/dashboard/admin/reviews`, `reviews` table + RLS, seeded mock data)
...
- [x] Proposals admin CRUD at `/dashboard/admin/proposals` (list/filter/search, create, edit, delete draft, send)
- [x] `/proposal/[token]` secure token-gated client route (unguessable `public_token`; no client slug in URL)
- [ ] Digital signature capture (SignWell API or canvas-based)
- [x] Proposal approval auto-creates project record (idempotent upsert on `projects.proposal_id`)
...
- [-] Integrate Cal.com or Calendly embed on `/contact` — skipped in favour of an in-house calendar
- [x] Custom "pick a 15-min slot" calendar on `/contact#booking` (SAST working hours, double-booking guard, Turnstile + rate limit, booked slots surface in admin dashboard)
- [-] Auto-calendar invite + confirmation email on booking (confirmation email done; `.ics` invite still TODO)
...
### 3.2 Project Roadmap Tracker not sure if these are necessary
...
what i cooked
Navbars
- Admin sidebar: Proposals, Projects, Reviews added
- Public navbar: Testimonials + Case Studies added (footer + sitemap too)
Proposals & Projects (admin) — built at /dashboard/admin/proposals (list/filter/search, create, edit, detail, send-to-client) and /dashboard/admin/projects (status pipeline), all using the global CSS theme tokens, Button, Card, StatusBadge, .mg-input. Server actions now verify the admin session (the originals had no auth checks). Approving a proposal on the public /proposal/[token] link auto-creates its project.
Admin reviews — properly incorporated: now DB-backed (reviews table + RLS + realistic seeded SA-business reviews), approve/reject/reset persists and only approved ones show on /testimonials.
Testimonials mock data — realistic content created: lib/testimonials.ts, lib/client-logos.ts (wordmark marquee), lib/case-studies.ts (plumbing / fleet / panel-beating write-ups with results).
Migrations fixed
- Deleted duplicate proposal_items migration (table was created twice with conflicting schemas)
- Renamed files so order is correct — projects referenced proposals before it existed
- Added missing projects RLS, made the clients/meetings RLS re-runnable, added a unique index so two people can't book the same 15-min slot
- New: 20260915_04_motiongrid_create_reviews.sql
Calendar (operational) — /contact#booking: pick a day → open 15-min SAST slots (Mon–Fri 09:00–17:00) → book with Turnstile + rate limiting → confirmation/admin emails → booked slots vanish and appear under Upcoming meetings on the admin dashboard.
